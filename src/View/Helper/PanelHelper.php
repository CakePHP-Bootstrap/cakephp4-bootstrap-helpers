...
class PanelHelper extends Helper
{
     ...
     *
     * @var \Bootstrap\Utility\StackedStates
     */
    protected $_states;
    ...
    public function create($title = null, $options = [])
    {
        ...
        $out .= $this->formatTemplate('panelStart', [
            ...
            'attrs' => $this->templater()->formatAttributes(
                $options,
                ['body', 'type', 'collapsible', 'open', 'panel-count', 'title']
            ),
            'templateVars' => $options['templateVars'],
        ]);
     ...
     * @param array $options Array of options. See above.
     * @param array $titleOptions Header title options
     * @return string A formated opening tag for the panel header or the complete panel
     * header.
     */
    protected function _createHeader(string $title, array $options = [], array $titleOptions = []): string
    {
        $options += [
            'escape' => true,
            'templateVars' => [],
     ...
     * header.
     */
    public function header($info = null, array $options = []): string
    {
        if (is_array($info)) {
            $options = $info;
            $info = null;
